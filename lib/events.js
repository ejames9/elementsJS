...
var onceIE = function(event, el, callback) {
                                                                               //TEST:30 Test onceIE function.
  var callBack = function() {
    callback();
    document.getElementById(el).detachEvent('on' + event, callBack);
  };
  ...
  if (typeof el === 'string') {
    if (el[0] === '#') {
      el = el.slice(1);
      return (function() {
        document.getElementById(el).attachEvent('on' + event, callBack);
      }) ();
    } else if (el[0] === '.') {
      el = el.slice(1);
      return (function() {
        document.getElementsByClassName(el)[0].attachEvent('on' + event, callBack);
      }) ();
    } else {
      return (function() {
        document.getElementsByTagName(el).attachEvent('on' + event, callBack);
      }) ();
    }
  } else {
    return (function() {
