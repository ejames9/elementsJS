...
*/
 ...
 var err = require('./logger').err,
elements = require('./index'),
...
//TODO:10 List all functions that need to be added (.textContent, innerHTML etc.)
//TODO:0 Complete all functions for this object.
...
*/
  ...
  this.first = function(s) {                                                   //TEST: Make sure this works.
    let count = (s === 'all') ?
      this.el.firstChild
      :
  ...
  this.attrib = function(attrib, val) {                                        //TEST: Make sure this works
    ...
    // (r !== undefined) ?                                                     //TODO: See if you can make this work.
    //   return r
    // :
    //   return this;
    if (r !== undefined) {
  ...
  this.fore = function(el, ref=null) {                                    //TODO: Make sure this works.
    (ref === null) ?
  ...
  this.aft = function(el, ref=null) {                                    //TODO: Make sure this works.
    (ref === null) ?
  ...
  this.on = function(ev, callback) {
    elements.on(ev, this.el, callback);
    return this;
  };

  this.off = function(ev, callback) {
    elements.off(ev, this.el, callback);
    return this;
  };

  this.once = function(ev, callback) {
    elements.once(ev, this.el, callback);
    return this;
  };
  ...
  this.click = function(el, cb=null) {
    if (el.addEventListener) {
      events.click(el, cb);
    } else {
      events.clickIE(el, cb);
    }
    return this;
  };


  this.dblClick = function(el, cb=null) {
    if (el.addEventListener) {
      events.dblClick(el, cb);
    } else {
      events.dblClickIE(el, cb);
    }
    return this;
  };


  this.error = function(el, cb=null) {
    if (el.addEventListener) {
      events.error(el, cb);
    } else {
      events.errorIE(el, cb);
    }
    return this;
  };


  this.focus = function(el, cb=null) {
    if (el.addEventListener) {
      events.focus(el, cb);
    } else {
      events.focusIE(el, cb);
    }
    return this;
  };


  this.focusIn = function(el, cb=null) {
    if (el.addEventListener) {
      events.focusIn(el, cb);
    } else {
      events.focusInIE(el, cb);
    }
    return this;
  };


  this.focusOut = function(el, cb=null) {
    if (el.addEventListener) {
      events.focusOut(el, cb);
    } else {
      events.focusOut(el, cb);
    }
    return this;
  ...
  this.keyUp = function(el, cb=null) {
    if (el.addEventListener) {
      events.keyUp(el, cb);
    } else {
      events.keyUpIE(el, cb);
    }
    return this;
  };


  this.keyDown = function(el, cb=null) {
    if (el.addEventListener) {
      events.keyDown(el, cb);
    } else {
      events.keyDownIE(el, cb);
    }
    return this;
  };


  this.load = function(el, cb=null) {
    if (el.addEventListener) {
      events.load(el, cb);
    } else {
      events.loadIE(el, cb);
  ...
  this.unLoad = function(el, cb=null) {
    if (el.addEventListener) {
      events.unLoad(el, cb);
    } else {
      events.unLoadIE(el, cb);
    }
    return this;
  };


  this.mouse = function(el, cb=null) {
    if (el.addEventListener) {
      events.mouse(el, cb);
    } else {
      events.mouseIE(el, cb);
    }
    return this;
  };


  this.resize = function(el, cb=null) {
    if (el.addEventListener) {
      events.resize(el, cb);
    } else {
      events.resizeIE(el, cb);
    }
    return this;
  };


  this.scroll = function(el, cb=null) {
    if (el.addEventListener) {
      events.scroll(el, cb);
    } else {
      events.scrollIE(el, cb);
    }
    return this;
  };


  this.select = function(el, cb=null) {
    if (el.addEventListener) {
      events.select(el, cb);
    } else {
      events.selectIE(el, cb);
    }
    return this;
  };
